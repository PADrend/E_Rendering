...
void E_BufferObject::init(EScript::Namespace & lib) {
	...
	//! [ESMF] thisEObj BufferObject.clear(bufferTarget, [internalFormat=GL_R8, format=GL_RED, type=GL_BYTE])
	ES_MFUN(typeObject,BufferObject,"clear",1,4,(thisObj->clear(parameter[0].toUInt(), parameter[1].toUInt(0x8229), parameter[2].toUInt(0x1903), parameter[3].toUInt(0x1400)), thisEObj))
